...
void get_avg_sjl(int len, float* outs, float* codes, float* dates, float* zt_types)
{
	//MessageBoxA(nullptr, "1", "log", MB_OK);
	np::ndarray p_dates = np::from_data(dates, np::dtype::get_builtin<float>(),
		p::make_tuple(len),
	...
	np::ndarray p_outs = np::from_data(outs, np::dtype::get_builtin<float>(),
		...
		p::make_tuple(sizeof(float)),
		p::object());
	CHAODUAN.attr("get_avg_sjl")(p_outs, p::object(codes[0]), p_dates);
	//MessageBoxA(nullptr, "2", "log", MB_OK);
}

void get_median_sjl(int len, float* outs, float* codes, float* dates, float* zt_types)
{
	//MessageBoxA(nullptr, "1", "log", MB_OK);
	np::ndarray p_dates = np::from_data(dates, np::dtype::get_builtin<float>(),
		p::make_tuple(len),
	...
	np::ndarray p_outs = np::from_data(outs, np::dtype::get_builtin<float>(),
		...
		p::make_tuple(sizeof(float)),
		p::object());
	CHAODUAN.attr("get_median_sjl")(p_outs, p::object(codes[0]), p_dates);
	//MessageBoxA(nullptr, "2", "log", MB_OK);
}
...
PluginTCalcFuncInfo g_CalcFuncSets[] = 
{
	...
	{ 10,(pPluginFUNC)&get_avg_sjl },
	{ 11,(pPluginFUNC)&get_median_sjl },
	{0,NULL},
};
